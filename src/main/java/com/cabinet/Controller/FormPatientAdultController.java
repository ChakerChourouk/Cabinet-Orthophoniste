...
public class FormPatientAdultController {
...
private Stage stage;
public void switchToAjouterPatAdulte(ActionEvent event){
     DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String formattedDate = date.getValue().format(formatter);
Adulte adulte = new Adulte(nom.getText(),prenom.getText(),formattedDate,naiss.getText(),adresse.getText(),diplome.getText(),prof.getText(),Long.parseLong(tel.getText()));
int num=Main2Controller.gestion.genererNumeroDossier();
adulte.setNumeroDossier(num);
...
Alert confirmation = new Alert(AlertType.CONFIRMATION);
   ...
   confirmation.showAndWait().ifPresent(response -> {
       if (response == ButtonType.OK) {
        Main2Controller.gestion.ajouterPatient(adulte);
